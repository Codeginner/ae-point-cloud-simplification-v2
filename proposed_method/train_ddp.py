...
class TNet(nn.Module):
    """Spatial Transformer Network (T-Net) — identik dengan script/train_pointnet.py."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:   # x: (B, k, N)
        ...
        x = x + torch.eye(self.k, device=x.device).unsqueeze(0)
        return x                                           # (B, k, k)


class PointNetCls(nn.Module):
    """PointNet + T-Net — arsitektur identik dengan script/train_pointnet.py
    agar checkpoint pointnet_cls_mn40.pth dapat di-load tanpa error."""
